...
class INDLoss(nn.Module):
    def forward(self,cls2,cls3,labelcls2,labelcls3):


        b,a,w,h,aa=cls2.size()
        for i in range(b):
            label2=labelcls2[i,0,:,:]
            label2=label2.view(-1)
            pos = label2.data.eq(1).nonzero(as_tuple =False).squeeze().cuda()
            score2=F.softmax(cls2[i,0,:,:,:], dim=2)[:,:,1]
            score2=score2.view(-1,1)
            score2=torch.index_select(score2,0,pos)

            score3=cls3[i,0,:,:]
            score3=F.softmax(cls3.view(-1), dim=0)
            score3=score2.view(-1,1)
            score3=torch.index_select(score3,0,pos)        
            score=(score2+score3)/2  #0.5 0.7
            idx=score.argmax()
            best_pos=torch.index_select(pos,0,idx)


            score3=cls3.view(-1).cpu().detach().numpy()*cfg.TRACK.w3
